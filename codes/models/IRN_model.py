...
class IRNModel(BaseModel):
    def __init__(self, opt):
        ...
        if opt['dist']:
            ...
            self.netG = DataParallel(self.netG)  # 是数据并行的一种方法，可以帮助我们在多个GPU上复制同一个模型。
        # print network
        # self.print_network()
        self.load()
    ...
    def feed_data_LQ(self, data):
        # print(data)
        self.img = data['LQ'].to(self.device)
        print('xxxxxxxxxxxxxxxxx',self.img.size())
        return self.img
    ...
    def optimize_parameters(self):
        ...
        l_back_rec = self.loss_backward(self.real_H, LR)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            # self.fake_H = self.netG(x=self.forw_L, rev=True)[:, :3, :, :]
            self.fake_H, _ = self.netG(x=self.forw_L, rev=True)
    ...
    def upscale(self, LR_img):
        ...
        self.netG.eval()
        with torch.no_grad():
            HR_img, _ = self.netG(x=LR_img, rev=True)
            HR_img = HR_img[:, :3, :, :]
        self.netG.train()
    ...
    def print_network(self):
        s, n = self.get_network_description(self.netG)  # 是一个用来获取网络结构描述的函数
